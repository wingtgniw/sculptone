...
describe('useAutosave', () => {
  ...
  it('변경 후 saveProject에 현재 project를 인수로 전달한다', async () => {
    renderHook(() => useAutosave())
    await act(async () => { vi.advanceTimersByTime(800) }) // 최초 마운트: 저장 없음
    act(() => { useStore.getState().setProject({ ...useStore.getState().project }) })
    await act(async () => { vi.advanceTimersByTime(800) })
  ...
  it('언마운트 시 대기 중인 저장이 취소된다', async () => {
    ...
    act(() => { useStore.getState().setProject({ ...useStore.getState().project }) })
    await act(async () => { vi.advanceTimersByTime(400) }) // 400ms 진행(디바운스 미완)
    unmount()
    await act(async () => { vi.advanceTimersByTime(800) }) // 추가 800ms 진행
    expect(mockSave).not.toHaveBeenCalled()
  })
})
